...
frequencies2 <- colSums(dfBengal) / (2 * 13)
frequencies2 <- as.numeric(frequencies2)
freq_Bengal <- data.frame(markers2, frequencies2)
...
freq_Bengal$frequencies2[freq_Bengal$frequencies2 == 0] <- 0.00001
freq_Amur$frequencies[freq_Amur$frequencies == 1] <- 0.99999
freq_Bengal$frequencies2[freq_Bengal$frequencies2 == 1] <- 0.99999
```
...
k = 13 # control number of individuals
  ...
  for(i in 1:k){
    for(j in 1:m){
      if(dfAmur[i,top_indices[j]] == 2){
        ppl2value[i] = ppl2value[i]*(2-deltaAmur[i,top_indices[j]])*freq_Bengal[top_indices[j], 2]*freq_Bengal[top_indices[j], 2]
      }
      else if(dfAmur[i,top_indices[j]] == 0){
        ppl2value[i] = ppl2value[i]*(2-deltaAmur[i,top_indices[j]])*(1-freq_Bengal[top_indices[j], 2])*(1-freq_Bengal[top_indices[j], 2])
      }
      else {
        ppl2value[i] = ppl2value[i]*(2-deltaAmur[i,top_indices[j]])*freq_Bengal[top_indices[j], 2]*(1-freq_Bengal[top_indices[j], 2])
      }
    }
  }
  ...
  for(i in 1:k){
    for(j in 1:m){
      if(dfBengal[i,top_indices[j]] == 2){
        ppl1value[i] = ppl1value[i]*(2-deltaBengal[i,top_indices[j]])*freq_Amur[top_indices[j], 2]*freq_Amur[top_indices[j], 2]
      }
      else if(dfBengal[i,top_indices[j]]== 0){
        ppl1value[i] = ppl1value[i]*(2-deltaBengal[i,top_indices[j]])*(1-freq_Amur[top_indices[j], 2])*(1-freq_Amur[top_indices[j], 2])
      }
      else {
        ppl1value[i] = ppl1value[i]*(2-deltaBengal[i,top_indices[j]])*freq_Amur[top_indices[j], 2]*(1-freq_Amur[top_indices[j], 2])
      }
    }
  }
  ppl2value = numeric(k)
  ...
  for(i in 1:k){
    for(j in 1:m){
      if(dfBengal[i,top_indices[j]] == 2){
        ppl2value[i] = ppl2value[i]*(2-deltaBengal[i,top_indices[j]])*freq_Bengal[top_indices[j], 2]*freq_Bengal[top_indices[j], 2]
      }
      else if(dfBengal[i,top_indices[j]] == 0){
        ppl2value[i] = ppl2value[i]*(2-deltaBengal[i,top_indices[j]])*(1-freq_Bengal[top_indices[j], 2])*(1-freq_Bengal[top_indices[j], 2])
      }
      else {
        ppl2value[i] = ppl2value[i]*(2-deltaBengal[i,top_indices[j]])*freq_Bengal[top_indices[j], 2]*(1-freq_Bengal[top_indices[j], 2])
      }
    }
  }
  ...
  for(i in 1:k){
    for(j in 1:m){
      if(dfAmur[i,random_indices[j]] == 2){
        ppl2value[i] = ppl2value[i]*(2-deltaAmur[i,random_indices[j]])*freq_Bengal[random_indices[j], 2]*freq_Bengal[random_indices[j], 2]
      }
      else if(dfAmur[i,random_indices[j]] == 0){
        ppl2value[i] = ppl2value[i]*(2-deltaAmur[i,random_indices[j]])*(1-freq_Bengal[random_indices[j], 2])*(1-freq_Bengal[random_indices[j], 2])
      }
      else {
        ppl2value[i] = ppl2value[i]*(2-deltaAmur[i,random_indices[j]])*freq_Bengal[random_indices[j], 2]*(1-freq_Bengal[random_indices[j], 2])
      }
    }
  }
  ...
  for(i in 1:k){
    for(j in 1:m){
      if(dfBengal[i,random_indices[j]] == 2){
        ppl1value[i] = ppl1value[i]*(2-deltaBengal[i,random_indices[j]])*freq_Amur[random_indices[j], 2]*freq_Amur[random_indices[j], 2]
      }
      else if(dfBengal[i,random_indices[j]]== 0){
        ppl1value[i] = ppl1value[i]*(2-deltaBengal[i,random_indices[j]])*(1-freq_Amur[random_indices[j], 2])*(1-freq_Amur[random_indices[j], 2])
      }
      else {
        ppl1value[i] = ppl1value[i]*(2-deltaBengal[i,random_indices[j]])*freq_Amur[random_indices[j], 2]*(1-freq_Amur[random_indices[j], 2])
      }
    }
  }
  ppl2value = numeric(k)
  ...
  for(i in 1:k){
    for(j in 1:m){
      if(dfBengal[i,random_indices[j]] == 2){
        ppl2value[i] = ppl2value[i]*(2-deltaBengal[i,random_indices[j]])*freq_Bengal[random_indices[j], 2]*freq_Bengal[random_indices[j], 2]
      }
      else if(dfBengal[i,random_indices[j]]== 0){
        ppl2value[i] = ppl2value[i]*(2-deltaBengal[i,random_indices[j]])*(1-freq_Bengal[random_indices[j], 2])*(1-freq_Bengal[random_indices[j], 2])
      }
      else {
        ppl2value[i] = ppl2value[i]*(2-deltaBengal[i,random_indices[j]])*freq_Bengal[random_indices[j], 2]*(1-freq_Bengal[random_indices[j], 2])
      }
    }
  }
  matrix2 <- matrix(0, nrow = 1, ncol = k)
...
```
```{r}
write.csv(result, file = "Amur_Bengal.csv", row.names = FALSE)
```
...
ggsave("plot_Amur_Bengal.png",width = 10, height = 8, dpi = 300)
```
